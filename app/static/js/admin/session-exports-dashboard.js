...
async function downloadCurrentPage() {
  console.log('[DOWNLOAD] Download current page clicked');
  console.log('[DOWNLOAD] filteredSessionExportSessions:', filteredSessionExportSessions);

  // Collect all downloadable session IDs from current page
  const sessionIds = [];

  filteredSessionExportSessions.forEach(user => {
    console.log('[DOWNLOAD] User:', user.username, 'session1_can_download:', user.session1_can_download, 'session2_can_download:', user.session2_can_download);
  ...
  console.log('[DOWNLOAD] Collected session IDs:', sessionIds);
...
async function downloadAllSessions() {
  console.log('[DOWNLOAD] Download all sessions clicked');

  // Confirm before starting (we don't know the count yet, but that's ok)
  const confirmed = confirm(`Download ALL sessions matching current filters?\n\nThis may take a while for large datasets.`);
  if (!confirmed) return;

  try {
    // Build URL with export_all flag and filters
    let url = `/admin/facial-analysis/export/bulk?export_all=true`;
    if (currentSessionExportSearchQuery) {
      url += `&q=${encodeURIComponent(currentSessionExportSearchQuery)}`;
    }
    url += `&sort_by=${currentSessionExportSortBy}&sort_order=${currentSessionExportSortOrder}`;

    console.log('[DOWNLOAD] Downloading all sessions from:', url);
    ...
    downloadAllButton.innerHTML = `
      <svg class="animate-spin h-4 w-4 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    // Fetch the zip file directly from backend
    const response = await fetch(url, {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      a.href = downloadUrl;
      a.download = `bulk_sessions_all_${new Date().getTime()}.zip`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(downloadUrl);
      document.body.removeChild(a);

      alert('Successfully downloaded all sessions!');
    } else {
    ...
    alert(`Error downloading sessions: ${error.message}`);
    ...
    if (downloadAllButton) {
      ...
      downloadAllButton.innerHTML = `
        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 4m4 4v12"></path>
        </svg>
        Download All
      `;
    }
