...
function setupRefreshDetection(sessionId) {
  ...
  window.addEventListener("beforeunload", function () {
    ...
    sessionStorage.setItem(
      "refreshingSession_" + sessionId,
      Date.now().toString()
    );
    console.log(
      "Refresh flag set for session " + sessionId + " at",
      Date.now()
    );
  });
  ...
  window.addEventListener("DOMContentLoaded", function () {
    const refreshTimestamp = sessionStorage.getItem(
      "refreshingSession_" + sessionId
    );
    if (refreshTimestamp) {
      ...
      if (timeDiff < 5000) {
        ...
        const isLikelyRefresh =
          navigation.length > 0 && navigation[0].type === "reload";

        if (!isFromAssessmentPage && isLikelyRefresh) {
          console.log(
            "Resetting session " + sessionId + " due to recent page refresh"
          );
          ...
          fetch(`/assessment/reset-session-on-refresh/${sessionId}`, {
            ...
            .catch((error) => {
              console.log(
                "Session reset failed, redirecting to main menu anyway:",
                error
              );
          ...
          console.log(
            "Ignoring refresh flag - normal navigation or not a true refresh"
          );
        }
      } else {
        console.log(
          "Ignoring old refresh flag for session " + sessionId + ", too old:",
          timeDiff,
          "ms"
        );
      }
